...
let build_locale_getter locale_getter =
  match Js.Nullable.toOption locale_getter with
  | Some s -> s
  | None -> "lc"
...
let rec build_expression locale_getter = function
  ...
  | FunctionRef (name, params) ->
      "(" ^ name ^ (build_params params) ^ " " ^ (build_locale_argument
      locale_getter) ^ ")"
  | Select (selector, pattern_array) -> build_select locale_getter selector pattern_array
  | BuiltInRef (name, subject, params) ->
      "(" ^ (builtin_name name) ^ " " ^ (build_expression locale_getter subject) ^ " " ^
      (build_builtin_params name params) ^ " " ^ (build_locale_getter
      locale_getter) ^ ")"

    and build_pattern_element locale_getter = (function
  | TextElement text -> "{js|" ^ text ^ "|js}"
  | Expression expr -> build_expression locale_getter expr)
...
and build_select_number locale_getter selector pattern_array_with_default =
  ...
  let selector =
    "(" ^ built_selector
    ^ ", Fluent.plural_rule " ^ build_expression locale_getter selector_without_builtin ^ " lc"
    ^ type_param ^ ")" in
  let patterns =
...
let build_function_head fn locale_getter =
  ...
  let added_unit = match (params, locale_argument) with
  | "", "" -> "()"
  | _ -> "" in
  "let " ^ fn.name
  ^ params
  ^ " " ^ locale_argument ^ added_unit
  ^ " =\n"
...
let build_function default_lc locale_getter fn =
  ...
  let body =
    ...
    | 0 -> ""
    | 1 -> build_pattern locale_getter (snd (Belt.Array.getExn fn.bodies 0))
    | _ ->
        let patterns =
        ...
        in
        build_switch locale_getter (build_locale_getter locale_getter) (*[||]*) patterns default_pattern
  in
