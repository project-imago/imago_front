...
type msg =
  | Login of (string, string) Tea.Result.t
...
type model = { client : Matrix.client }
let init () =
  Js.log "init";
  let client = Matrix.new_client () in 
  ({ client }, Tea_promise.result (Matrix.login client) login)
...
let update model = function (* These should be simple enough to be self-explanatory, mutate the model based on the message, easy to read and follow *)
  | Increment -> (model, Tea.Cmd.none)
  | Decrement -> (model, Tea.Cmd.none)
  | Reset -> (model, Tea.Cmd.none)
  | Set v -> (model, Tea.Cmd.none)
...
let main =
  standardProgram { (* The beginnerProgram just takes a set model state and the update and view functions *)
    init; (* Since model is a set value here, we call our init function to generate that value *)
    update;
    view;
